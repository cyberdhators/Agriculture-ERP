...
credit scoring, USSD, IVR, voice advisories, government database integration,
blockchain, IoT, satellite imagery analysis, e-learning courses, certificates
and quizzes, helpdesk ticketing, predictive analytics.
...
- WhatsApp integration, deliverable (o). Conditional on Meta business
  verification of the client's account. Do not start until I say verification
  has been granted.
...
their assigned state, and neither sees beyond it.
...
only. Any new variable goes in `.env.example` and you tell me its name.
...
**Git.** Never push to `main`. Feature branches and pull requests only.
...
- Test, type-check and lint results, as pass or fail
