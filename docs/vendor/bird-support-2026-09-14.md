...
The sender was created at `20:03:48`. The Liberian failures start at `20:13` and
the South Sudan delivery is at `20:23`, so one registration appears to have
bound within twenty minutes and the other had not bound after ten.
...
3. **Is this propagation?** If registrations bind asynchronously, what is the
   expected window, and is there a field that distinguishes "filed" from "live
   at the carrier"? `approved` with `next: []` reads as complete.
...
6. **South Sudan, which is the actual target.** MTN South Sudan delivered. What
   ...
   expect the Liberian pattern there?
