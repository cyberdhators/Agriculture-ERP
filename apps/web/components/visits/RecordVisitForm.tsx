...
export function RecordVisitForm({
  ...
  /** Live-mode caseload state, so an empty picker can say why (loading / error). */
  names?: VisitNames;
          ...
          hint={
            names?.error
              ...
              : names?.loading
                ...
                : farmers.length === 0
                  ? 'No farmers in your caseload yet. Register one from the field desk first.'
                  : undefined
          }
        ...
        >
          {(ids) => (
            <Select
              ...
              disabled={Boolean(names?.loading)}
              onChange={(e) => set('farmer_id', e.target.value)}
            >
              <option value="">{names?.loading ? 'Loading…' : 'Choose a farmer…'}</option>
              {farmers.map((f) => (
